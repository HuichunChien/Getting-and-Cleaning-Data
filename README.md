...
# v 1. Merges the training and the test sets to create one data set.
# v 2. Extracts only the measurements on the mean and standard deviation for each measurement. 
# v 3. Uses descriptive activity names to name the activities in the data set
# v 4. Appropriately labels the data set with descriptive variable names. 
# v 5. From the data set in step 4, creates a second, independent tidy data set with the average of each variable for each activity and each subject.
...
train_y <- read.table("./UCI HAR Dataset/train/Y_train.txt")    # types of activity performed on train dataset: WALKING, WALKING_UPSTAIRS, WALKING_DOWNSTAIRS, SITTING, STANDING, LAYING 
...
names(totdata) <- c("subject_id", "activity_id", features_names, measurement_id ) # measurement_id is crasted separately in measurement_id.R

####################################################################################################

# The following steps are meant to creat descriptive variable names for Means_Stds tidy dataset
...
# The descriptive names are created as follows.
names(Means_Stds_tidy) <- c("subject_id", "activity_id", mean_names, std_names ) # measurement_id is crasted separately in measurement_id.R
